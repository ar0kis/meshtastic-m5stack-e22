...
void initM5StackPanel(Adafruit_ILI9341& display) {
  ...
  display.sendCommand(ILI9341_DFUNCTR, display_function,
                      sizeof(display_function));
  display.sendCommand(ILI9341_GMCTRP1, gamma_positive,
                      sizeof(gamma_positive));
  display.sendCommand(ILI9341_GMCTRN1, gamma_negative,
...
void M5StackILI9341Display::setColor(Color c) {
  switch (c) {
    case DARK:   _color = kBackground; break;
    case LIGHT:  _color = kText;       break;
    case RED:    _color = kError;      break;
    case GREEN:  _color = kSuccess;    break;
    case BLUE:   _color = kPanel;      break;
    case YELLOW: _color = kWarning;    break;
    case ORANGE: _color = kAccent;     break;
    default:     _color = kText;       break;
  }
  unicode.setForegroundColor(_color);
}
...
void M5StackILI9341Display::drawTextEllipsized(int x, int y,
                                               int max_width,
                                               const char* str) {
  char text[256];
  ...
  while (len > 0 && getTextWidth(text) > max_width - ellipsis_width) {
    do {
      --len;
             ...
             (static_cast<uint8_t>(text[len]) & 0xC0) == 0x80);
    text[len] = 0;
  }
...
void M5StackILI9341Display::translateUTF8ToBlocks(char* dest,
                                                  const char* src,
                                                  size_t dest_size) {
  if (dest_size == 0) return;
  size_t len = strlen(src);
  const bool truncated = len >= dest_size;
  if (truncated) len = dest_size - 1;
  if (truncated) {
    while (len > 0 &&
           (static_cast<uint8_t>(src[len]) & 0xC0) == 0x80) {
      --len;
    }
  }
  memcpy(dest, src, len);
...
void M5StackILI9341Display::drawXbm(int x, int y, const uint8_t* bits,
                                    int w, int h) {

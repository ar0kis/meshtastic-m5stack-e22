...
void MyMesh::onDiscoveredContact(ContactInfo &contact, bool is_new, uint8_t path_len, const uint8_t *path)
{
    ...
    if (!is_new)
        dirty_contacts_expiry =
            futureMillis(LAZY_CONTACTS_WRITE_DELAY); // only schedule lazy write for contacts that are in contacts[]
}
...
{
    if (max_num > ADVERT_PATH_TABLE_SIZE)
        max_num = ADVERT_PATH_TABLE_SIZE;
...
void MyMesh::queueMessage(const ContactInfo &from, uint8_t txt_type, mesh::Packet *pkt, uint32_t sender_timestamp,
...
void MyMesh::onChannelDataRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint16_t data_type, const uint8_t *data,
                               size_t data_len)
{
    if (data_len > MAX_CHANNEL_DATA_LENGTH) {
        MESH_DEBUG_PRINTLN("onChannelDataRecv: dropping payload_len=%d exceeds frame limit=%d", (uint32_t)data_len,
                           (uint32_t)MAX_CHANNEL_DATA_LENGTH);
        return;
    }
...
uint8_t MyMesh::onContactRequest(const ContactInfo &contact, uint32_t sender_timestamp, const uint8_t *data, uint8_t len,
                                 uint8_t *reply)
{
...
MyMesh::MyMesh(mesh::Radio &radio, mesh::RNG &rng, mesh::RTCClock &rtc, SimpleMeshTables &tables, DataStore &store,
    ...
    : BaseChatMesh(radio, *new ArduinoMillis(), rng, rtc, *new StaticPoolPacketManager(16), tables), _serial(NULL),
      telemetry(MAX_PACKET_PAYLOAD - 4), _store(&store), _ui(ui)
{
    _iter_started = false;
    _cli_rescue = false;
    offline_queue_len = 0;
...
#if defined(ESP32) && defined(INIT_TJM_RADIO_PROFILE_V1)
    // Apply this build's Tyumen profile once, so stale Companion preferences from
    // an older installation cannot silently override the requested radio setup.
    // The marker keeps later settings made in the app fully persistent.
    FILESYSTEM *primary_fs = _store->getPrimaryFS();
    if (!primary_fs->exists("/tjm_prof_v1")) {
        _prefs.freq = LORA_FREQ;
        _prefs.bw = LORA_BW;
        _prefs.sf = LORA_SF;
        _prefs.cr = LORA_CR;
        ...
        File marker = primary_fs->open("/tjm_prof_v1", FILE_WRITE);
        if (marker) {
...
bool MyMesh::isValidClientRepeatFreq(uint32_t f) const
{
    for (int i = 0; i < sizeof(repeat_freq_ranges) / sizeof(repeat_freq_ranges[0]); i++) {
        ...
        if (f >= r->lower_freq && f <= r->upper_freq)
            return true;
    }
    return false;
}
...
void MyMesh::handleCmdFrame(size_t len)
{
    if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
        ...
        out_frame[i++] =
            (_prefs.telemetry_mode_env << 4) | (_prefs.telemetry_mode_loc << 2) | (_prefs.telemetry_mode_base); // v5+
        ...
        if (recipient && (txt_type == TXT_TYPE_PLAIN || txt_type == TXT_TYPE_CLI_DATA)) {
            ...
            if (txt_type == TXT_TYPE_CLI_DATA) {
                msg_timestamp =
                    getRTCClock()
                        ->getCurrentTimeUnique(); // Use node's RTC instead of app timestamp to avoid tripping replay protection
            ...
            writeErrFrame(recipient == NULL ? ERR_CODE_NOT_FOUND
        ...
        if (repeat && !isValidClientRepeatFreq(freq)) {
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
        } else if (freq >= 150000 && freq <= 2500000 && sf >= 5 && sf <= 12 && cr >= 5 && cr <= 8 && bw >= 7000 && bw <= 500000) {
            _prefs.sf = sf;
            _prefs.cr = cr;
...
{
    return _mgr->getOutboundCount(0xFFFFFFFF) > 0 || dirty_contacts_expiry != 0;
}

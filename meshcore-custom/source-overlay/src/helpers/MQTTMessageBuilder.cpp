...
int MQTTMessageBuilder::buildStatusMessage(const char *origin, const char *origin_id, const char *model,
                                           const char *firmware_version, const char *radio, const char *client_version,
                                           const char *status, const char *timestamp, char *buffer, size_t buffer_size,
                                           int battery_mv, int uptime_secs, int errors, int queue_len, int noise_floor,
                                           int tx_air_secs, int rx_air_secs, int recv_errors, float temperature_c,
                                           float humidity_percent, float pressure_hpa, float altitude_m, float dew_point_c)
{
    ...
    if (battery_mv >= 0 || uptime_secs >= 0 || errors >= 0 || queue_len >= 0 || noise_floor > -999 || tx_air_secs >= 0 ||
        rx_air_secs >= 0 || recv_errors >= 0) {
    ...
    if (!isnan(temperature_c) || !isnan(humidity_percent) || !isnan(pressure_hpa) || !isnan(altitude_m) || !isnan(dew_point_c)) {
        JsonObject environment = root.createNestedObject("environment");
        if (!isnan(temperature_c))
            environment["temperature_c"] = roundf(temperature_c * 10.0f) / 10.0f;
        if (!isnan(humidity_percent))
            environment["humidity_percent"] = roundf(humidity_percent * 10.0f) / 10.0f;
        if (!isnan(pressure_hpa))
            environment["pressure_hpa"] = roundf(pressure_hpa * 10.0f) / 10.0f;
        if (!isnan(altitude_m))
            environment["altitude_m"] = roundf(altitude_m * 10.0f) / 10.0f;
        if (!isnan(dew_point_c))
            environment["dew_point_c"] = roundf(dew_point_c * 10.0f) / 10.0f;
    }
...
int MQTTMessageBuilder::buildPacketMessage(const char *origin, const char *origin_id, const char *timestamp,
                                           const char *direction, const char *time, const char *date, int len, int packet_type,
                                           const char *route, int payload_len, const char *raw, float snr, int rssi,
                                           const char *hash, const char *path, char *buffer, size_t buffer_size)
{
...
int MQTTMessageBuilder::buildRawMessage(const char *origin, const char *origin_id, const char *timestamp, const char *raw,
...
int MQTTMessageBuilder::buildPacketJSON(mesh::Packet *packet, bool is_tx, const char *origin, const char *origin_id,
                                        Timezone *timezone, char *buffer, size_t buffer_size)
{
    if (!packet)
        return 0;
    ...
    if (packet->isRouteDirect() && packet->path_len > 0) {
        snprintf(path_str, sizeof(path_str), "path_%dx%d_%db", (int)packet->getPathHashCount(), (int)packet->getPathHashSize(),
                 (int)packet->getPathByteLen());
    }

    return buildPacketMessage(origin, origin_id, timestamp, is_tx ? "tx" : "rx", time_str, date_str, packet->getRawLength(),
                              packet_type, route_str, packet->payload_len, raw_hex,
                              ...
                              hash_str, packet->isRouteDirect() ? path_str : nullptr, buffer, buffer_size);
}

int MQTTMessageBuilder::buildPacketJSONFromRaw(const uint8_t *raw_data, int raw_len, mesh::Packet *packet, bool is_tx,
                                               const char *origin, const char *origin_id, float snr, float rssi,
                                               Timezone *timezone, char *buffer, size_t buffer_size)
{
    if (!packet || !raw_data || raw_len <= 0)
        return 0;
    ...
    return buildPacketMessage(origin, origin_id, timestamp, is_tx ? "tx" : "rx", time_str, date_str,
...
int MQTTMessageBuilder::buildRawJSON(mesh::Packet *packet, const char *origin, const char *origin_id, Timezone *timezone,
                                     char *buffer, size_t buffer_size)
{
    if (!packet)
        return 0;

...
// #region agent log
static void agentLogHeap(const char *location, const char *message, const char *hypothesisId, size_t free_h, size_t max_alloc,
                         unsigned long internal_free, unsigned long spiram_free)
{
    char buf[320];
...
// Only force-disconnect main broker after this many consecutive publish failures (reduces heap fragmentation from
...
void MQTTBridge::formatMqttStatusReply(char *buf, size_t bufsize, const NodePrefs *prefs)
{
    ...
    q = b->_queue_count;
#endif
    snprintf(buf, bufsize, "> msgs: %s, broker: %s, us: %s, eu: %s, queue: %d", msgs, broker, us, eu, q);
}

MQTTBridge::MQTTBridge(NodePrefs *prefs, mesh::PacketManager *mgr, mesh::RTCClock *rtc, mesh::LocalIdentity *identity)
    : BridgeBase(prefs, mgr, rtc), _mqtt_client(nullptr), _active_brokers(0), _queue_count(0), _last_status_publish(0),
      _last_status_retry(0), _status_interval(300000), // 5 minutes default
      _ntp_client(_ntp_udp, "pool.ntp.org", 0, 60000), _last_ntp_sync(0), _ntp_synced(false), _ntp_sync_pending(false),
      _timezone(nullptr), _last_raw_len(0), _last_snr(0), _last_rssi(0), _last_raw_timestamp(0), _analyzer_us_enabled(false),
      _analyzer_eu_enabled(false), _identity(identity), _analyzer_us_client(nullptr), _analyzer_eu_client(nullptr),
      _config_valid(false), _cached_has_brokers(false), _cached_has_analyzer_servers(false), _last_memory_check(0),
      _skipped_publishes(0), _last_fragmentation_recovery(0), _fragmentation_pressure_since(0), _last_critical_check_run(0),
      _last_no_broker_log(0), _last_config_warning(0), _dispatcher(nullptr), _radio(nullptr), _board(nullptr), _ms(nullptr),
      _last_wifi_check(0), _last_wifi_status(WL_DISCONNECTED), _wifi_status_initialized(false), _wifi_disconnected_time(0),
      _last_wifi_reconnect_attempt(0), _wifi_reconnect_backoff_attempt(0), _main_broker_reconnect_backoff_attempt(0),
      _analyzer_us_reconnect_backoff_attempt(0), _analyzer_eu_reconnect_backoff_attempt(0)
#ifdef ESP_PLATFORM
      ,
      _packet_queue_handle(nullptr), _mqtt_task_handle(nullptr), _raw_data_mutex(nullptr), _mqtt_task_stack(nullptr),
...
// PSRAM diagnostic - helps debug memory fragmentation on boards with external RAM
#ifdef BOARD_HAS_PSRAM
    {
        ...
        MQTT_DEBUG_PRINTLN("PSRAM: found=%s, size=%u, free=%u", psram_available ? "YES" : "NO", psram_size, psram_free);
        if (!psram_available) {
        ...
        MQTT_DEBUG_PRINTLN("PSRAM: internal_free=%u, internal_max_alloc=%u", heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
...
// Create FreeRTOS queue; use PSRAM storage when available (plan §5)
    ...
    if (_packet_queue_storage != nullptr) {
        _packet_queue_handle =
            xQueueCreateStatic(MAX_QUEUE_SIZE, sizeof(QueuedPacket), _packet_queue_storage, &_packet_queue_struct);
    } else {
    ...
    MQTT_DEBUG_PRINTLN("Analyzer servers - US: %s, EU: %s", _analyzer_us_enabled ? "enabled" : "disabled",
...
#define MQTT_TASK_PRIORITY 1
    ...
    BaseType_t create_result = xTaskCreatePinnedToCore(mqttTask, "MQTTBridge", MQTT_TASK_STACK_SIZE, this, MQTT_TASK_PRIORITY,
                                                       &_mqtt_task_handle, MQTT_TASK_CORE);
...
// #endregion
        ...
        if (_status_enabled) {
            ...
            if (!has_destinations) {
                ...
                if (_last_status_retry != 0) {
                    unsigned long retry_elapsed =
                        (now >= _last_status_retry) ? (now - _last_status_retry) : (ULONG_MAX - _last_status_retry + now + 1);
                    ...
                    if (_last_status_publish == 0) {
                        ...
                        unsigned long elapsed = (now >= _last_status_publish) ? (now - _last_status_publish)
                ...
                if (should_publish) {
                    ...
                    if (_last_status_publish != 0) {
                        unsigned long elapsed = (now >= _last_status_publish) ? (now - _last_status_publish)
                                                                              : (ULONG_MAX - _last_status_publish + now + 1);
                        MQTT_DEBUG_PRINTLN("Status publish timer expired (elapsed: %lu ms, interval: %lu ms)", elapsed,
                                           _status_interval);
                    } else {
...
bool MQTTBridge::isConfigValid(const NodePrefs *prefs)
{
    ...
    if (strlen(prefs->mqtt_server) == 0 || strcmp(prefs->mqtt_server, "your-mqtt-broker.com") == 0) {
        return false;
    }
...
void MQTTBridge::checkConfigurationMismatch()
{
    ...
    if (_prefs->bridge_pkt_src == 0 && _packets_enabled && !_tx_enabled) {
        ...
        if (_last_config_warning == 0 || (now - _last_config_warning > CONFIG_WARNING_INTERVAL)) {
            MQTT_DEBUG_PRINTLN("MQTT: Configuration mismatch detected! bridge.source=tx (logTx) but mqtt.tx=off. Packets will "
                               "not be published. Run 'set bridge.source rx' or 'set mqtt.tx on' to fix.");
...
void MQTTBridge::loop()
{
    ...
    if (_status_enabled) {
        ...
        if (_status_enabled && _last_status_publish != 0) {
            unsigned long time_since_last_success =
                (now >= _last_status_publish) ? (now - _last_status_publish) : (ULONG_MAX - _last_status_publish + now + 1);
            ...
            if (time_since_last_success > MAX_FAILURE_TIME_MS) {
                ...
                if (now - last_reinit_log > 300000) { // Log every 5 minutes max
                    MQTT_DEBUG_PRINTLN(
                        "CRITICAL: Status publish has been failing for %lu ms (>%lu ms), forcing MQTT session reinitialization",
...
void MQTTBridge::onPacketReceived(mesh::Packet *packet)
{
    ...
    bool has_valid_brokers =
        _config_valid || (_analyzer_us_enabled && _analyzer_us_client) || (_analyzer_eu_enabled && _analyzer_eu_client);
...
{
    if (!_initialized || !_packets_enabled || !_tx_enabled)
        return;
...
{
    // Check if MQTT server is configured (not default placeholder)
    if (strlen(_prefs->mqtt_server) == 0 || strcmp(_prefs->mqtt_server, "your-mqtt-broker.com") == 0) {
        return false;
    }
...
void MQTTBridge::runCriticalMemoryCheckAndRecovery()
{
    ...
    spiram_f = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
#endif
    agentLogHeap("MQTTBridge.cpp:runCriticalMemoryCheckAndRecovery", "critical_memory_check", "H1_H4", free_h, max_alloc,
                 internal_f, spiram_f);
#endif
    ...
    unsigned long required_window_ms =
        (max_alloc < PRESSURE_THRESHOLD_CRITICAL) ? PRESSURE_WINDOW_CRITICAL_MS : PRESSURE_WINDOW_MODERATE_MS;
    if (_fragmentation_pressure_since != 0 && (now - _fragmentation_pressure_since) >= required_window_ms &&
        (now - _last_fragmentation_recovery) >= RECOVERY_THROTTLE_MS) {
        _last_fragmentation_recovery = now;
        _fragmentation_pressure_since = 0;
        MQTT_DEBUG_PRINTLN("Fragmentation recovery: recreating MQTT clients (max_alloc=%d, pressure %lu min)", (int)max_alloc,
                           (unsigned long)(required_window_ms / 60000));
        recreateMqttClientsForFragmentationRecovery();
    }
}
#endif
...
void MQTTBridge::processPacketQueue()
{
    ...
    while (processed < max_per_loop) {
        ...
        publishPacket(queued.packet, queued.is_tx, queued.has_raw_data ? queued.raw_data : nullptr,
                      queued.has_raw_data ? queued.raw_len : 0, queued.has_raw_data ? queued.snr : 0.0f,
...
bool MQTTBridge::publishStatus()
{
    ...
    if (!isIATAValid()) {
        ...
        if (now - last_iata_warning > 300000) {
            MQTT_DEBUG_PRINTLN(
                "MQTT: Cannot publish status - IATA code not configured (current: '%s'). Please set mqtt.iata via CLI.", _iata);
    ...
    snprintf(radio_info, sizeof(radio_info), "%.6f,%.1f,%d,%d", _prefs->freq, _prefs->bw, _prefs->sf, _prefs->cr);
...
#ifdef M5STACK_CORE_E22
    temperature_c = sensors.getTemperatureC();
    humidity_percent = sensors.getHumidityPercent();
    pressure_hpa = sensors.getPressureHpa();
    altitude_m = sensors.getAltitudeM();
    dew_point_c = sensors.getDewPointC();
#endif
    ...
    int len = MQTTMessageBuilder::buildStatusMessage(_origin, origin_id,
                                                     _board_model,      // model - now dynamic!
                                                     _firmware_version, // firmware version
                                                     radio_info,
                                                     client_version, // client version
                                                     "online", timestamp, json_buffer, STATUS_JSON_BUFFER_SIZE, battery_mv,
                                                     uptime_secs, errors,
                                                     _queue_count, // Use current queue length
                                                     noise_floor, tx_air_secs, rx_air_secs, recv_errors, temperature_c,
                                                     humidity_percent, pressure_hpa, altitude_m, dew_point_c);
                ...
                if (_brokers[i].enabled && _brokers[i].connected) {
                    ...
                    if (publish_result > 0) {
                        ...
                        bool should_disconnect =
                            (s_consecutive_main_publish_failures >= MAIN_CLIENT_DISCONNECT_FAILURE_THRESHOLD);
                        static unsigned long last_status_publish_fail_log = 0;
                        unsigned long now = millis();
                        if (now - last_status_publish_fail_log > 60000) {
                            MQTT_DEBUG_PRINTLN("Status publish failed (result=%d), failures=%d%s", publish_result,
                                               s_consecutive_main_publish_failures,
...
void MQTTBridge::publishPacket(mesh::Packet *packet, bool is_tx, const uint8_t *raw_data, int raw_len, float snr, float rssi)
{
    if (!packet)
        return;
    ...
    if (!isIATAValid()) {
        ...
        if (now - last_iata_warning > 300000) {
            MQTT_DEBUG_PRINTLN(
                "MQTT: Cannot publish packet - IATA code not configured (current: '%s'). Please set mqtt.iata via CLI.", _iata);
            last_iata_warning = now;
        }
        return;
    }
...
// Threshold: Max alloc < 60KB indicates severe fragmentation
    ...
    if (now - _last_memory_check > 5000) { // Check every 5 seconds
        ...
        if (max_alloc < 60000) { // Less than 60KB max alloc = severe fragmentation
            ...
            if (now - last_skip_log > 60000) { // Log every minute
                MQTT_DEBUG_PRINTLN("MQTT: Skipping publish due to memory pressure (Max alloc: %d, skipped: %d)", max_alloc,
                                   _skipped_publishes);
                last_skip_log = now;
            }
    ...
    if (raw_data && raw_len > 0) {
        ...
        len = MQTTMessageBuilder::buildPacketJSONFromRaw(raw_data, raw_len, packet, is_tx, _origin, origin_id, snr, rssi,
        ...
        len = MQTTMessageBuilder::buildPacketJSONFromRaw(_last_raw_data, _last_raw_len, packet, is_tx, _origin, origin_id,
                                                         _last_snr, _last_rssi, _timezone, active_buffer, active_buffer_size);
    } else {
        // Fallback to reconstructed packet data
        len =
            MQTTMessageBuilder::buildPacketJSON(packet, is_tx, _origin, origin_id, _timezone, active_buffer, active_buffer_size);
    }
            ...
            for (int i = 0; i < MAX_MQTT_BROKERS_COUNT; i++) {
                ...
                if (_brokers[i].enabled && _brokers[i].connected && _mqtt_client->connected()) {
                    ...
                    if (publish_result > 0) {
                        ...
                        s_consecutive_main_publish_failures++;
                        // Only disconnect after several consecutive failures to avoid heap fragmentation from
                        // disconnect/reconnect storms
                        bool should_disconnect =
                            (s_consecutive_main_publish_failures >= MAIN_CLIENT_DISCONNECT_FAILURE_THRESHOLD);
                        ...
                        if (now - last_publish_fail_log > 60000) {
                            MQTT_DEBUG_PRINTLN("Publish failed (result=%d), failures=%d%s", publish_result,
                                               s_consecutive_main_publish_failures,
                                               should_disconnect ? ", forcing reconnect" : "");
...
void MQTTBridge::publishRaw(mesh::Packet *packet)
{
    ...
    if (!isIATAValid()) {
        ...
        if (now - last_iata_warning > 300000) {
            MQTT_DEBUG_PRINTLN(
                "MQTT: Cannot publish raw packet - IATA code not configured (current: '%s'). Please set mqtt.iata via CLI.",
                _iata);
            last_iata_warning = now;
        }
        return;
    ...
    // Build raw message
    int len = MQTTMessageBuilder::buildRawJSON(packet, _origin, origin_id, _timezone, active_buffer, active_buffer_size);
                ...
                if (_brokers[i].enabled && _brokers[i].connected && _mqtt_client->connected()) {
                    ...
                    if (publish_result > 0) {
                        ...
                        if (now - last_raw_publish_fail_log > 60000) {
                            MQTT_DEBUG_PRINTLN("Raw publish failed (result=%d), failures=%d%s", publish_result,
                                               s_consecutive_main_publish_failures,
                                               should_disconnect ? ", forcing reconnect" : "");
...
void MQTTBridge::setBroker(int broker_index, const char *host, uint16_t port, const char *username, const char *password,
                           bool enabled)
{
    if (broker_index < 0 || broker_index >= MAX_MQTT_BROKERS_COUNT)
        return;
...
bool MQTTBridge::createAuthToken()
{
    ...
    if (_analyzer_us_enabled && _auth_token_us) {
        if (JWTHelper::createAuthToken(*_identity, "mqtt-us-v1.letsmesh.net", 0, expires_in, _auth_token_us, AUTH_TOKEN_SIZE,
    ...
    if (_analyzer_eu_enabled && _auth_token_eu) {
        if (JWTHelper::createAuthToken(*_identity, "mqtt-eu-v1.letsmesh.net", 0, expires_in, _auth_token_eu, AUTH_TOKEN_SIZE,
                                       owner_key, client_version, email)) {
    ...
    if (us_token_created || eu_token_created) {
        MQTT_DEBUG_PRINTLN("Auth tokens created (US:%s EU:%s)", us_token_created ? "yes" : "no", eu_token_created ? "yes" : "no");
    }
...
#ifdef MQTT_MEMORY_DEBUG
        // #region agent log
        agentLogHeap("MQTTBridge.cpp:2142", "after_new_analyzer_us_client", "H4", ESP.getFreeHeap(), ESP.getMaxAllocHeap(),
...
#ifdef MQTT_MEMORY_DEBUG
        // #region agent log
        agentLogHeap("MQTTBridge.cpp:2182", "after_new_analyzer_eu_client", "H4", ESP.getFreeHeap(), ESP.getMaxAllocHeap(),
                     heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
#ifdef BOARD_HAS_PSRAM
                     heap_caps_get_free_size(MALLOC_CAP_SPIRAM)
#else
...
void MQTTBridge::publishStatusToAnalyzerClient(PsychicMqttClient *client, const char *server_name)
{
    ...
    if (!isIATAValid()) {
        ...
        // Only log this warning every 5 minutes to avoid spam
        if (now - last_iata_warning > 300000) {
            MQTT_DEBUG_PRINTLN("MQTT: Cannot publish status to analyzer - IATA code not configured (current: '%s'). Please set "
                               "mqtt.iata via CLI.",
                               _iata);
            last_iata_warning = now;
        }
    ...
    // JSON buffer in PSRAM when available (plan §4)
    static const size_t ANALYZER_STATUS_JSON_SIZE = 1024;
    ...
    int len = MQTTMessageBuilder::buildStatusMessage(_origin, origin_id,
                                                     ...
                                                     radio_info,
                                                     client_version, // client version
                                                     "online", timestamp, json_buffer, ANALYZER_STATUS_JSON_SIZE, battery_mv,
                                                     uptime_secs, errors,
                                                     _queue_count, // Use current queue length
                                                     noise_floor, tx_air_secs, rx_air_secs, recv_errors, temperature_c,
                                                     humidity_percent, pressure_hpa, altitude_m, dew_point_c);
...
void MQTTBridge::maintainAnalyzerConnections()
{
    ...
    if (_analyzer_us_enabled && _analyzer_us_client) {
        ...
        if (token_needs_renewal && can_attempt_renewal) {
            ...
            if (_auth_token_us && JWTHelper::createAuthToken(*_identity, "mqtt-us-v1.letsmesh.net", 0, 86400, _auth_token_us,
                                                             AUTH_TOKEN_SIZE, owner_key, client_version, email)) {
                unsigned long expires_in = 86400; // 24 hours
                _token_us_expires_at = time_synced ? (current_time + expires_in) : 0;
                ...
                bool old_token_expired_or_imminent = !time_synced || (old_token_expires_at == 0) ||
    ...
    if (_analyzer_eu_enabled && _analyzer_eu_client) {
        ...
        if (token_needs_renewal && can_attempt_renewal) {
            ...
            // Renew the token (only if buffer was allocated)
            if (_auth_token_eu && JWTHelper::createAuthToken(*_identity, "mqtt-eu-v1.letsmesh.net", 0, 86400, _auth_token_eu,
                                                             AUTH_TOKEN_SIZE, owner_key, client_version, email)) {
                unsigned long expires_in = 86400; // 24 hours
                _token_eu_expires_at = time_synced ? (current_time + expires_in) : 0;
...
void MQTTBridge::setStatsSources(mesh::Dispatcher *dispatcher, mesh::Radio *radio, mesh::MainBoard *board,
...
{
    MQTT_DEBUG_PRINTLN("Memory: Free=%d, Max=%d, Queue=%d/%d", ESP.getFreeHeap(), ESP.getMaxAllocHeap(), _queue_count,
                       MAX_QUEUE_SIZE);
}
